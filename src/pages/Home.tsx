...
import React, { useState, type ReactElement, type ReactNode, type FormEvent } from "react";
...
const Home = (): ReactElement => {
  ...
  const handleAddCurtain = (): void => {
    ...
    const newCurtain = { id: curtains.length + 1, dimensions: defaultDimensions};
    setCurtains([...curtains, newCurtain]);
  }

  const handleOnChangeCurtain = (curtainId: number, segmentId: string, length: number, angle: number): void => {
    ...
    <div className="flex w-full max-w-7xl mx-auto min-h-screenNav bg-white">
      <div className="flex w-full justify-center items-center bg-orange-300">
        ...
        {curtains.map(curtain => (
          ...
          <button type="button" className="bg-green-500 p-4 text-white" onClick={() => {handleAddCurtain()}}>Add Square</button>
          { curtains.length > 0 &&
            <button type="submit" className="bg-blue-500 p-4 text-white">Submit</button>
          }
        </form>
