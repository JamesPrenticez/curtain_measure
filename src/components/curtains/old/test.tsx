...
const Line = ({ segments, stroke, strokeWidth }: LineProps): ReactElement => {
  const pathData = segments.reduce<number[]>(
    (acc, { length, angle }, index) => {
      ...
      const angleX =
        newX - (strokeWidth / 2) * Math.cos((angle * Math.PI) / 180);
      const angleY =
        newY - (strokeWidth / 2) * Math.sin((angle * Math.PI) / 180);
  ...
  const angleCircles = segments.map(({ length, angle }, index) => {
    ...
    return (
      <circle
        ...
        d={`M ${x} ${y} L ${x + radius * Math.cos(startAngle)} ${
          y + radius * Math.sin(startAngle)
        } A ${radius} ${radius} 0 0 1 ${x + radius * Math.cos(endAngle)} ${
